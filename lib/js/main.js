    ...
    $(document).ready(function () {
        ...
        $('form#add-post-form').validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: 'POST',
                    url: guest_post_data.ajaxurl,
                    data: {
                        action: 'post_form_submit',
                        security: guest_post_data.nonce,
                        post_data: $("form#add-post-form").serialize(),
                        content: content
                    },
                    success: function (res) {
                        if (res.success) {
                            $(".response").empty().html('<div class="alert alert-success" role="alert">' + guest_post_data.sucess + '</div>').removeClass('d-none');
                        } else {
                            $(".response").empty().html('<div class="alert alert-danger" role="alert">' + guest_post_data.error + '</div>').removeClass('d-none');
                        }
                        $(".guest-post-featured-image").empty();
                    ...
                    error: function (xhr, textStatus, errorThrown) {
                        $(".response").empty().html('<div class="alert alert-danger" role="alert">' + guest_post_data.error + '</div>');
                        return false;
                    }
                });
                return false;
